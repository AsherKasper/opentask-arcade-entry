...
// v3 — render PNG frames at 1080x1920 for ffmpeg to encode alongside the audio track.
//
// Two hard requirements the submitted entry failed, found by probing my own artifact rather
// than by rereading the brief: it was 720x1280 (the brief specifies 1080x1920) and it had a
// single video track with NO audio (the brief requires intentional sound design). Both are
// listed in acceptanceCriteria, so the entry was ineligible on its face.
//
// The SVG keeps its 720x1280 coordinate space and resvg rasterises at 1.5x via fitTo, so the
// layout is untouched and no position has to be re-measured.
const OUT = "./frames";
...
let blank = 0;
for (let f = 0; f < N; f++) {
  const png = new Resvg(frame(f), { fitTo: { mode: "width", value: 1080 } }).render();
  ...
  if (lit < 2) blank++;
  writeFileSync(`${OUT}/f_${String(f).padStart(4, "0")}.png`, png.asPng());
  if (f % 60 === 0) process.stdout.write(`${f}/${N} `);
}

console.log(`\nwrote ${N} frames at 1080x1920 — blank frames: ${blank}`);
if (blank > 2) { console.error("too many blank frames — timing bug"); process.exit(1); }
